...
8. Decide whether educator and retrospector recommendations warrant invocation.
   ...
   follow the provider adapter, and preserve one educator identity throughout.
   In direct-thread mode, present the educator's nonterminal initial message and
   remain active waiting. In coordinator-relay mode, present each returned
   `awaiting-human` turn, end your turn so Lei regains the input UI, then resume
   the same educator with Lei's next response. Only a terminal education status
   closes the session.
