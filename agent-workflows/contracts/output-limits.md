# Output limits

Default caps keep handoffs readable and force each agent to form a coherent
narrative. The coordinator may set a smaller or larger limit when the task
requires it.

| Output | Default cap |
|---|---:|
| Context packet | 1,200 words |
| Child result | 800 words |
| Review result | 1,200 words |
| Education brief | 1,000 words |
| Current `progress.html` narrative | 1,500 words |
| Topic learner profile | 2,000 words |

Link to evidence rather than copying verbose logs. The coordinator publishes
current and archived teaching material in the configured canonical locations;
supporting children return bounded payloads instead of writing those locations.
Output caps do not justify omitting a blocker, correctness issue, or user
decision.
