...
6. When the workflow can create or monitor a PR, the coordinator starts one
   ...
   PRs and starts the Reviewer. The Reviewer alone invokes the adapter's
   cross-provider primary review route. A plan-level review may run earlier for
   a risky design under the same cross-provider rule.
